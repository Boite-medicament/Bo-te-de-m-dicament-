...
int duree1;                               // duration from the launch of the program until the last rotation of the servomotor
int duree2;                               // duration since the launch of the program until now 
int delai;                                // duration between the last rotation of the servomotor until now 
int intervalle;                           // time during which the servomotor is blocked
...
void loop() {
  ...
  if(distance < 10)                                                                       // if someone put his in front of the sensor 
  {
    ...
    duree2 = millis();
    delai = duree2 - duree1;
    if  (delai >= intervalle)                                                             // the interval has been respected, the user must take his medicine
    {
      duree1 = millis();
    ...
    else                                                                                  // the interval isn't respected, the servomotor don't turn 
    {
      display.printFixedN (0, 16, "                         ", STYLE_BOLD, FONT_SIZE_2X); // erase the old message
      display.printFixedN (0, 16, " DEJA PRIS ", STYLE_BOLD, FONT_SIZE_2X);               // display the sentence on the screen
      delay(1000);
    }
  }
  ...
  else                                                                                    // the screen will show if the medicine must be taken or has already been 
  {
    ...
    if (delai >= intervalle )
    {
      ...
      display.printFixedN (0, 32, "                   ", STYLE_BOLD, FONT_SIZE_2X);
      display.printFixed (0, 16, "   Prenez votre  ", STYLE_BOLD);  
      display.printFixedN (0, 32, "medicament", STYLE_BOLD, FONT_SIZE_2X);
      delay(100);
    }
    else
    {
      display.printFixedN (0, 16, "Medicament Pris     ", STYLE_BOLD, FONT_SIZE_2X);          
      delay(100);
    }
  }
}
